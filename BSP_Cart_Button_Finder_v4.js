(function () {
   ...
   function initializeMainScript(parsedMapiData = null, eventRequestId = null) {
     ...
     // Build URL
     let buyflowUrl = `https://shop.brightspeed.com/uas/?utm_medium=DAP&utm_source=Clearlink&affprog=clearlink&salescode=${salescode}&cookietime=30day${
     ...
     function processScopedLinks(url) {
       ...
       if (telAnchor && !cartAnchor) {
         ...
         try {
           ...
           if (telButton) {
             ...
             `;
             console.log('[BSP Cart Button Finder] Applied button styling to button inside tel: link');
           } else {
             console.log('[BSP Cart Button Finder] No button found inside tel: link');
           }
         } catch (e) {
           console.error('[BSP Cart Button Finder] Failed to apply styling to button inside tel: link', e);
         }
         ...
         try {
          ...
          try {
            ...
            console.log('[BSP Cart Button Finder] Injected buyflow button before tel: link');
          } catch (innerError) {
       ...
       // NEW FUNCTIONALITY: If no tel buttons found, insert button below last .leshen-typography-body
       if (!telAnchor && !cartAnchor) {
         console.log('[BSP Cart Button Finder] No tel or cart links found, checking for .leshen-typography-body elements');
        
         // Find all .leshen-typography-body elements within the content
         const typographyElements = content.querySelectorAll('.leshen-typography-body');
        
         if (typographyElements.length > 0) {
          // Get the last .leshen-typography-body element
          ...
          // Check if there's a .leshen-list element after the last .leshen-typography-body
          let targetElement = lastTypographyElement;
          let nextElement = lastTypographyElement.nextElementSibling;
          let foundList = false;
          
          // Look for .leshen-list in subsequent siblings
          while (nextElement) {
            if (nextElement.classList.contains('leshen-list')) {
              targetElement = nextElement;
              foundList = true;
              console.log('[BSP Cart Button Finder] Found .leshen-list after .leshen-typography-body, will insert after list');
              break;
            }
          ...
          if (!foundList) {
            console.log('[BSP Cart Button Finder] No .leshen-list found after .leshen-typography-body, will insert after typography element');
          }
          ...
          try {
            // Insert the button after the target element (either .leshen-list or .leshen-typography-body)
            targetElement.insertAdjacentHTML('afterend', newButtonHtml);
            console.log(`[BSP Cart Button Finder] Injected buyflow button after ${foundList ? '.leshen-list' : '.leshen-typography-body'} element`);
            
            // Add protection against DOM manipulation
            protectInjectedElements();
          } catch (e) {
            console.error('[BSP Cart Button Finder] Failed to inject button after target element', e);
            
            // Fallback: Try to insert at the end of the content
            try {
              const tempDiv = document.createElement('div');
              tempDiv.innerHTML = newButtonHtml;
              const buttonElement = tempDiv.firstElementChild;
              content.appendChild(buttonElement);
              console.log('[BSP Cart Button Finder] Injected buyflow button at end of content as fallback');
              protectInjectedElements();
            } catch (fallbackError) {
              console.error('[BSP Cart Button Finder] Even fallback insertion failed', fallbackError);
            }
          }
        } else {
          console.log('[BSP Cart Button Finder] No .leshen-typography-body elements found in content');
        } 
       } 
     }
     ...
     window.addEventListener('load', function () {
       ...
       if (valuesChanged) {
         ...
         buyflowUrl = `https://shop.brightspeed.com/uas/?utm_medium=DAP&utm_source=Clearlink&affprog=clearlink&salescode=${updatedValues.salescode}&cookietime=30day${
