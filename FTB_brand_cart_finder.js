(function () {
    console.log('[FTB Cart Button Finder] Script initialized');
    ...
    function initializeMainScript() {
        console.log('[FTB Cart Button Finder] Initializing main script');

        // Build URL
        const buyflowUrl = 'https://clearlink.go2cloud.org/aff_c?offer_id=3963&aff_id=1026';
        ...
        function processScopedLinks(url) {
            ...
            if (!heroEl) {
                console.log('[FTB Cart Button Finder] .leshen-hero not found');
                return;
            }
            const content = heroEl.querySelector(".content");
            if (!content) {
                console.log("[FTB Cart Button Finder] .content not found inside .leshen-hero");
                return;
            }

            // If we find an <input>, do nothing
            if (content.querySelector('input')) {
                console.log('[FTB Cart Button Finder] <input> found in scope; skipping');
                return;
            }
            ...
            if (telAnchor && cartAnchor) {
                try {
                    cartAnchor.href = url;
                    cartAnchor.dataset.FTBUpdated = '1';
                    console.log('[FTB Cart Button Finder] Updated existing /ftr-buy link in scope');
                } catch (e) {
                    console.error('[FTB Cart Button Finder] Failed updating /ftr-buy link', e);
                }
                return;
            }

            // If only tel exists: append the button anchor like in FTB_Global_4_Responsive_Button.js
            if (telAnchor && !cartAnchor) {
                // Avoid duplicating if already injected
                if (content.querySelector('a.leshen-link-button-wrapper[data-FTB-injected="1"]')) {
                    console.log('[FTB Cart Button Finder] Button already injected; skipping');
                    return;
                }
             ...
             `;
                        console.log('[FTB Cart Button Finder] Applied button styling to button inside tel: link');
                    } else {
                        console.log('[FTB Cart Button Finder] No button found inside tel: link');
                    }
                } catch (e) {
                    console.error('[FTB Cart Button Finder] Failed to apply styling to button inside tel: link', e);
                }

                // Add a unique ID to make it easier to find and re-inject if removed
                const buttonId = 'FTB-injected-button-' + Math.floor(Math.random() * 10000);

                // Create a more resilient button that's harder to remove
                const newButtonHtml = `
         <a id="${buttonId}" class="leshen-link leshen-link-button-wrapper css-1s55t5c e9y95tf0 FTB-cart-link" href="${url}" data-FTB-injected="1" visibility="All devices" style="
             display: block !important;
             visibility: visible !important;
             opacity: 1 !important;
         ">
             ...
             ">
                ...
                try {
                    ...
                    try {
                        ...
                        console.log('[FTB Cart Button Finder] Injected buyflow button before tel: link');
                    } catch (innerError) {
                        console.warn('[FTB Cart Button Finder] Primary insertion method failed, trying alternative', innerError);
                        ...
                        console.log('[FTB Cart Button Finder] Injected buyflow button using alternative method');
                    }

                    // Add protection against DOM manipulation
                    protectInjectedElements();
                } catch (e) {
                    console.error('[FTB Cart Button Finder] All insertion methods failed', e);
                    ...
                    try {
                        ...
                        if (parentSection) {
                            ...
                            console.log('[FTB Cart Button Finder] Injected buyflow button at end of section as last resort');
                            protectInjectedElements();
                        }
                    } catch (lastError) {
                        console.error('[FTB Cart Button Finder] Even last resort insertion failed', lastError);
                    }
                }
                return;
            }
            ...
            if (!telAnchor && cartAnchor) {
                try {
                    cartAnchor.href = url;
                    cartAnchor.dataset.FTBUpdated = '1';
                    console.log('[FTB Cart Button Finder] Updated /ftr-buy link (no tel present)');
                } catch (e) {
                    console.error('[FTB Cart Button Finder] Failed updating /ftr-buy link (no tel)', e);
                }
            }
        }
        ...
        function processAllCartLinks(url) {
            console.log('[FTB Cart Button Finder] Searching for all cart links on the page');
            ...
            if (allCartLinks.length === 0) {
                console.log('[FTB Cart Button Finder] No cart links found on the page');
                return;
            }

            console.log(`[FTB Cart Button Finder] Found ${allCartLinks.length} cart links on the page`);

            allCartLinks.forEach((link, index) => {
                try {
                    // Skip links that were already processed by processScopedLinks
                    if (link.dataset.FTBUpdated === '1') {
                        console.log(`[FTB Cart Button Finder] Skipping already updated cart link #${index + 1}`);
                        return;
                    }

                    link.href = url;
                    link.dataset.FTBUpdated = '1';
                    console.log(`[FTB Cart Button Finder] Updated cart link #${index + 1}`);
                } catch (e) {
                    console.error(`[FTB Cart Button Finder] Failed updating cart link #${index + 1}`, e);
                }
            });
        }

        // Add a global verification flag to track successful URL modifications
        window.FTBCartButtonFinderSuccess = false;

        processScopedLinks(buyflowUrl);
        processAllCartLinks(buyflowUrl);

        // Check if initial application was successful
        const initialInjectedElements = document.querySelectorAll('[data-FTB-injected="1"]');
        const initialUpdatedElements = document.querySelectorAll('[data-FTB-updated="1"]');

        if (initialInjectedElements.length > 0 || initialUpdatedElements.length > 0) {
            console.log('[FTB Cart Button Finder] Initial URL modifications successful');
            window.FTBCartButtonFinderSuccess = true;
        }
        ...
        function scheduleReinjection(url) {
            ...
            allDelays.forEach(delay => {
                setTimeout(() => {
                    console.log(`[FTB Cart Button Finder] Scheduled re-injection check after ${delay}ms`);

                    // If we've already verified success and elements are still present, don't reapply
                    if (window.FTBCartButtonFinderSuccess) {
                        const currentElements = document.querySelectorAll('[data-FTB-injected="1"], [data-FTB-updated="1"]');
                        if (currentElements.length > 0) {
                            console.log('[FTB Cart Button Finder] URL modifications already verified successful, skipping re-injection');
                            return;
                        }
                    }
                    ...
                    if (injectedElements.length === 0 && updatedElements.length === 0 && alreadyUpdatedLinks.length === 0) {
                        ...
                        setTimeout(() => {
                            const newElements = document.querySelectorAll('[data-FTB-injected="1"], [data-FTB-updated="1"]');
                            if (newElements.length > 0) {
                                console.log('[FTB Cart Button Finder] Re-injection successful, marking as verified');
                                window.FTBCartButtonFinderSuccess = true;
                            }
                        }, 100);
                    }
            ...
            const intervalId = setInterval(() => {
                if (checkCount >= maxChecks) {
                    ...
                    console.log('[FTB Cart Button Finder] Ending continuous checks after 5 minutes');
                    return;
                }
                ...
                if (window.FTBCartButtonFinderSuccess) {
                    const currentElements = document.querySelectorAll('[data-FTB-injected="1"], [data-FTB-updated="1"]');
                    if (currentElements.length > 0) {
                        console.log('[FTB Cart Button Finder] Continuous check: URL modifications already verified successful, skipping');
                        return;
                    }
                }
                ...
                if (injectedElements.length === 0 && updatedElements.length === 0 && alreadyUpdatedLinks.length === 0) {
                    ...
                    setTimeout(() => {
                        const newElements = document.querySelectorAll('[data-FTB-injected="1"], [data-FTB-updated="1"]');
                        if (newElements.length > 0) {
                            console.log('[FTB Cart Button Finder] Continuous check re-injection successful, marking as verified');
                            window.FTBCartButtonFinderSuccess = true;
                        }
                    }, 100);
                }
        ...
        function protectInjectedElements() {
            const injectedElements = document.querySelectorAll('[data-FTB-injected="1"], [data-FTB-updated="1"]');

            injectedElements.forEach(element => {
                // Make it harder to remove the element
                const originalRemove = element.remove;
                element.remove = function () {
                    console.log('[FTB Cart Button Finder] Prevented removal of injected element');
                    return false;
                };
                ...
                element.setAttribute = function (name, value) {
                    if (name === 'style' && (value.includes('display: none') || value.includes('visibility: hidden') || value.includes('opacity: 0'))) {
                        console.log('[FTB Cart Button Finder] Prevented style attribute change that would hide element');
                        return false;
                    }
                ...
                element.classList.add = function (className) {
                    if (className.includes('hidden') || className.includes('invisible') || className.includes('removed')) {
                        console.log('[FTB Cart Button Finder] Prevented adding class that might hide element:', className);
                        return false;
                    }
        ...
        function setupMutationObserver(url) {
            console.log('[FTB Cart Button Finder] Setting up MutationObserver to monitor button removal');
            ...
            const observer = new MutationObserver((mutations) => {
                    ...
                    if (mutation.removedNodes.length > 0) {
                        ...
                        for (let i = 0; i < mutation.removedNodes.length; i++) {
                            ...
                            if (node.nodeType === 1) { // ELEMENT_NODE
                                if (node.dataset && node.dataset.FTBInjected === '1' || node.dataset && node.dataset.FTBUpdated === '1') {
                                    console.log('[FTB Cart Button Finder] Detected removal of our injected/updated element');
                                    needToReapply = true;
                                    break;
                                }

                                // Also check if any of the removed node's children had our data attribute
                                if (node.querySelector && node.querySelector('[data-FTB-injected="1"], [data-FTB-updated="1"]')) {
                                    console.log('[FTB Cart Button Finder] Detected removal of a container with our injected/updated elements');
                                    needToReapply = true;
                                    break;
                                }
                            }
                ...
                if (needToReapply) {
                    ...
                    if (window.FTBCartButtonFinderSuccess) {
                        console.log('[FTB Cart Button Finder] Elements removed but URL modifications already verified successful, skipping re-application');
                    } else {
                        console.log('[FTB Cart Button Finder] Re-applying button modifications after detected removal');
                        setTimeout(() => {
                            processScopedLinks(url);
                            processAllCartLinks(url);
                            ...
                            setTimeout(() => {
                                const newElements = document.querySelectorAll('[data-FTB-injected="1"], [data-FTB-updated="1"]');
                                if (newElements.length > 0) {
                                    console.log('[FTB Cart Button Finder] MutationObserver re-injection successful, marking as verified');
                                    window.FTBCartButtonFinderSuccess = true;
                                }
                            }, 100);
            ...
            if (document.body) {
                ...
                console.log('[FTB Cart Button Finder] MutationObserver started');
            } else {
                console.warn('[FTB Cart Button Finder] document.body not found, MutationObserver not started');
            }
            ...
            setInterval(() => {
                ...
                if (injectedElements.length === 0 && updatedElements.length === 0 && alreadyUpdatedLinks.length === 0) {
                    console.log('[FTB Cart Button Finder] Periodic check: No injected/updated elements found, re-applying');
                    processScopedLinks(url);
                    processAllCartLinks(url);
                    ...
                    setTimeout(() => {
                        const newElements = document.querySelectorAll('[data-FTB-injected="1"], [data-FTB-updated="1"]');
                        if (newElements.length > 0) {
                            console.log('[FTB Cart Button Finder] Periodic check re-injection successful, marking as verified');
                            window.FTBCartButtonFinderSuccess = true;
                        }
                    }, 100);
                }
        ...
        console.log('[FTB Cart Button Finder] Main script execution completed');
    }
